...
1. **`openvpn.username-claim`** - Extract username from a specific token claim (default: `preferred_username`)
2. **`openvpn.username-cel`** - Use a CEL expression to extract or transform the username from token claims
