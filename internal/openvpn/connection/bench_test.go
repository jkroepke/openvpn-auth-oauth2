...
func BenchmarkConnection(b *testing.B) {
	for name, message := range map[string]string{
		...
		b.Run(name, func(b *testing.B) {
			...
			for b.Loop() {
				client, err := connection.NewClient(new(config.Defaults), message)
				if err != nil {
					b.Fatal(err)
				}
