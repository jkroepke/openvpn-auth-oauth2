...
// OAuth2Start returns a [http.Handler] that starts the OAuth 2.0 authorization flow.
// It checks if the HTTP request has a valid state GET parameter generated by state.New.
// Optionally, it checks the HTTP client IP address against the VPN IP address.
// Finally, the HTTP request is delegated to [rp.AuthURLHandler].
func (c Client) OAuth2Start() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// check if the request has a state GET parameter.
...
// OAuth2Callback returns a [http.Handler] that handles the OAuth 2.0 callback.
...
func (c Client) postCodeExchangeHandler(logger *slog.Logger, session state.State, clientID string) rp.CodeExchangeCallback[*idtoken.Claims] {
	return func(
		...
		c.openvpn.AcceptClient(logger, session.Client, session.Client.CommonName)
